...
void checkSha1Hash(const char *message, size_t size, uint32_t *expectedValue, CuTest *tc) {
...
void TestSha1_JunkInHashContainer_CalculateHash(CuTest *tc) {
    ...
    uint32_t expectedValue[BLOCKS_USED_NUM] = {0xA9993E36, 0x4706816A, 0xBA3E2571, 0x7850C26C, 0x9CD0D89D};
    CuAssertIntEquals(tc, HASH_FUNC_E_OK, calculateSha1Hash(message, size, hash));
    CuAssertTrue(tc, memcmp(hash, expectedValue, BLOCKS_USED_NUM) == 0);
}
...
void TestSha1_EmptyCStr_CalculateHash(CuTest *tc) {
    char *message = "";
    size_t size = 0;
    uint32_t expectedValue[BLOCKS_USED_NUM] = {0xDA39A3EE, 0x5E6B4B0D, 0x3255BFEF, 0x95601890, 0xAFD80709};
...
void TestSha1_ShortCStr_CalculateHash(CuTest *tc) {
    ...
    uint32_t expectedValue[BLOCKS_USED_NUM] = {0xB6589FC6, 0xAB0DC82C, 0xF12099D1, 0xC2D40AB9, 0x94E8410C};
    checkSha1Hash(message, size, expectedValue, tc);
}
...
void TestSha1_LongCStr_CalculateHash(CuTest *tc) {
    ...
    size_t size = strlen(message);
    uint32_t expectedValue[BLOCKS_USED_NUM] = {0xF5CE5249, 0xF3985567, 0xC7B10DFB, 0xA2464D43, 0x49F5E126};
    checkSha1Hash(message, size, expectedValue, tc);
}
...
void TestSha1_NulInTheMiddle_CalculateHash(CuTest *tc) {
    ...
    size_t size = 3;
    uint32_t expectedValue[BLOCKS_USED_NUM] = {0x4A3DEC2D, 0x1F824528, 0x0855C42D, 0xB0EE4239, 0xF917FDB8};
    checkSha1Hash(message, size, expectedValue, tc);
}
...
void TestSha1_AllCharsUsed_CalculateHash(CuTest *tc) {
    ...
    uint32_t expectedValue[BLOCKS_USED_NUM] = {0x4916D6BD, 0xB7F78E68, 0x03698CAB, 0x32D1586E, 0xA457DFC8};
    checkSha1Hash(message, size, expectedValue, tc);
}
